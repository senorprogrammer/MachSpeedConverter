...
class MachSpeedCalculator {
    ...
    func mileh_to_mach(miles_per_hour: Double, outside_air_temp_f: Double) -> Double {
        let kmh = mile_to_km(miles_per_hour)
        let cel = fahr_to_celc(outside_air_temp_f)

        return kmh_to_mach(kmh, outside_air_temp_c: cel)
    }
    ...
    func mach_to_mileh(mach: Double, outside_air_temp_f: Double) -> Double {
        let cel = fahr_to_celc(outside_air_temp_f)
        let kmh = mach_to_kmh(mach , outside_air_temp_c: cel)

        return km_to_mile(kmh)
    }
...
print(NSString(format: "%.4f", calc.kmh_to_mach(100, outside_air_temp_c: 15))      == "0.0816")
print(NSString(format: "%.4f", calc.mileh_to_mach(100, outside_air_temp_f: 59))    == "0.1313")
print(NSString(format: "%.4f", calc.mach_to_kmh(0.8168, outside_air_temp_c: 15))   == "1000.6277")
print(NSString(format: "%.4f", calc.mach_to_mileh(0.1314, outside_air_temp_f: 59)) == "100.0446")
